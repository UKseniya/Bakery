...
<fmt:bundle basename="resources">
    ...
    <section>
        ...
        <c:choose>
            ...
            <c:otherwise>
                <p><fmt:message key="order.number"/><span class="tab1"><fmt:message key="order.customer"/></span>
                    <span class="tab2"> <fmt:message key="order.date.selected"/></span>
                    <span class="tab3"/><fmt:message key="status"/> </span> </p>
                <c:forEach var="order" items="${allOrders}">
                    <p><a href="javascript:unhide('${order.orderNumber}')" ;/> ${order.orderNumber} </a>
                        <span class="tab1">${order.user.firstName}&nbsp;${order.user.lastName}</span>
                        <span class="tab2">${order.requestedDate}</span><span class="tab3"/>${order.status}</span> </p>
                    <div id="${order.orderNumber}" class="hidden">
                        <i>&nbsp;&nbsp;&nbsp;&nbsp; <fmt:message key="name"/></i>&nbsp;&nbsp;&nbsp;&nbsp;/&nbsp;&nbsp;&nbsp;&nbsp;<i><fmt:message
                            key="quantity"/></i>
